...
from osm_hours import sunday_state  # noqa: E402
...
def render(bases, lang):
    L = ['/* ==========================================================================',
         ...
         '     sunday     — the hours string was too complex to read safely.',
    ...
    for b in bases:
        ...
        for p in b["pois"]:
            ...
            for k in ("hours", "sunday", "website", "phone", "cuisine", "parking"):
                if k in p:
...
def main():
    ...
    for b in cfg["bases"]:
        ...
        pois = [p for p in (to_poi(e, lang) for e in raw) if p]
    ...
    Path(out_path).write_text(render(bases, lang), encoding="utf-8")
    ...
    print(f"{total_review} review items outstanding. Run tests/validate_data.py "
          f"to see them, and do NOT ship while `hours` or `sunday` are unresolved.")
